...
export function useAgentSelection(projectKey: string) {
  ...
  const inProject = currentPathname === `/projects/${projectKey}` || currentPathname.startsWith(`/projects/${projectKey}/`);
  const agentId = inProject ? currentParams.get("agent") || null : null;
  const mode = inProject && (agentId || currentParams.get("assistant") === "1") ? "assistant" : "inspect";
  const navigate = useCallback((id: string | null, open: boolean) => {
    ...
    router.push(currentPathname + (query ? `?${query}` : ""), { scroll: false });
  }, [currentPathname, currentParams, router]);
