...
function MessageContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex min-w-0 flex-col gap-1.5 group-data-[align=end]/message:items-end", className)}
      {...props}
    />
  );
}
...
function MessageHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex max-w-full min-w-0 items-center px-3 text-xs font-medium text-muted-foreground", className)}
      {...props}
    />
  );
}
