...
describe("knowledge_context_index workflow node", () => {
  ...
  it("continues with an explicit unavailable catalog when no service is configured", async () => {
    const parsed = parseWorkflowGraph({
      version: 4,
      variables: [],
      nodes: [
    ...
    expect(result.kind).toBe("completed");
    expect(result.bag.frame?.pins["catalog::datasets"]).toEqual([]);
    expect(result.bag.frame?.pins["catalog::usageHint"]).toContain("not configured");
  });
});
