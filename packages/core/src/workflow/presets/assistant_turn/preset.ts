...
export const assistantTurnPreset: WorkflowPreset = {
  ...
  body: [
    "Inputs: session (AssistantSession), message (string), optional windowSize (default 5, min 1), and optional knowledgeDataset (CortexDB dataset key).",
    ...
    "The decision node uses the versioned Assistant role manifest and chooses reply, clarify, retrieve, delegate, or resume. Retrieval is bounded and uses only read-only Query, typed knowledge_context_index, knowledge_search, file_list, or file_read nodes. Knowledge retrieval explicitly selects lookupDatasetKey from the catalog or configured dataset before vector search.",
