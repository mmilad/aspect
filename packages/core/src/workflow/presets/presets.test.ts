...
describe("workflow presets", () => {
  it("lists mutation packs and create_step, parks authoring presets", () => {
    ...
    expect(presets.some((preset) => preset.presetKey === "knowledge_capture_file")).toBe(true);
  ...
  it("assigns a closed kind to every catalog pack", () => {
    ...
    expect(listWorkflowPresets().find((preset) => preset.presetKey === "knowledge_index_project")?.kind).toBe(
      "user"
    );
  });
