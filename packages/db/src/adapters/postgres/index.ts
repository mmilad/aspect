...
export class PostgresStorage implements Storage {
  ...
  private async updateRun(input: { id: string; status?: WorkflowRunStatus; bag?: JsonRecord; error?: string | null; finished?: boolean }): Promise<void> { await this.executor.query(`UPDATE pp_workflow_runs SET status=COALESCE($1,status),bag_json=COALESCE($2,bag_json),error=$3,finished_at=CASE WHEN $4 THEN CURRENT_TIMESTAMP ELSE finished_at END WHERE id=$5`, [input.status ?? null, input.bag ?? null, input.error ?? null, Boolean(input.finished), input.id]); }
