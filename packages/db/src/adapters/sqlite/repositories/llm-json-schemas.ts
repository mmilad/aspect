...
import type { LlmJsonSchemaRecord } from "../../../contracts/llm-json-schemas";
export type { LlmJsonSchemaRecord } from "../../../contracts/llm-json-schemas";

import type { EnsureLlmJsonSchemasOptions } from "../../../contracts/llm-json-schemas";
export type { EnsureLlmJsonSchemasOptions } from "../../../contracts/llm-json-schemas";

import type { EnsureLlmJsonSchemasResult } from "../../../contracts/llm-json-schemas";
export type { EnsureLlmJsonSchemasResult } from "../../../contracts/llm-json-schemas";

import type { CreateLlmJsonSchemaInput } from "../../../contracts/llm-json-schemas";
export type { CreateLlmJsonSchemaInput } from "../../../contracts/llm-json-schemas";
...
function insertVersion(db: DatabaseSync, schemaId: string, version: number, schemaJson: string): void {
  db.prepare(`INSERT INTO llm_json_schema_versions (id, schema_id, version, schema_json) VALUES (?, ?, ?, ?)`).run(
    `ljsv_${randomUUID()}`,
    schemaId,
    version,
    schemaJson
...
function getByKey(db: DatabaseSync, key: string, projectKey = "PLAN"): LlmJsonSchemaRecord | null {
  const row = db
    .prepare(
...
function list(db: DatabaseSync, projectKey = "PLAN"): LlmJsonSchemaRecord[] {
  const rows = db
    .prepare(
      `SELECT s.id, s.project_id, s.key, s.title, s.description, s.schema_json, s.status
       FROM llm_json_schemas s
...
function create(db: DatabaseSync, input: CreateLlmJsonSchemaInput): LlmJsonSchemaRecord {
  ...
  const existing = getByKey(db, key, projectKey);
  if (existing) {
  ...
  const created = getByKey(db, key, projectKey);
  if (!created) {
...
function ensureUnscoped(db: DatabaseSync, options: EnsureLlmJsonSchemasOptions = {}): EnsureLlmJsonSchemasResult {
  ...
  for (const preset of presets) {
    const existing = getByKey(db, preset.key, projectKey);
...
export default llmJsonSchemas;

function ensure(db: DatabaseSync, options: EnsureLlmJsonSchemasOptions = {}): EnsureLlmJsonSchemasResult {
 const projectKey = options.projectKey ?? "PLAN";
 const presets = LLM_JSON_SCHEMA_PRESETS.filter(p => !options.only?.length || options.only.includes(p.key));
 const force = options.force || process.env.PROJECTPLANER_PRESETS_FORCE === "1";
 const needsWrite = findProjectId(db, projectKey) && presets.some(p => {
  const existing = getByKey(db, p.key, projectKey);
  return !existing || (force && JSON.stringify(existing.schema) !== JSON.stringify(p.schema));
 });
 if (!needsWrite || db.isTransaction) return ensureUnscoped(db, options);
 db.exec("BEGIN IMMEDIATE");
 try { const result = ensureUnscoped(db, options); db.exec("COMMIT"); return result; }
