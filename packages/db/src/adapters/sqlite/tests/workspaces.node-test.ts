import { testController } from "./support";
...
test("managed workspace lifecycle, duplicate ownership, restart, archive and recovery", async () => {
  ...
  try {
    ...
    assert.equal((await readProjectWorkspace(testController(db), "CODE")).workspace, null);
    const results = await Promise.allSettled([
      provisionProjectWorkspace(testController(db), "CODE", { mode: "create" }),
      provisionProjectWorkspace(testController(db), "CODE", { mode: "create" })
    ]);
    assert.equal(results.filter((r) => r.status === "fulfilled").length, 1);
    let state = await readProjectWorkspace(testController(db), "CODE");
    ...
    assert.equal((await readProjectWorkspace(testController(db), "CODE")).workspace?.repositoryPath, location);
    await fs.writeFile(path.join(location, "hello.txt"), "hello");
    assert.equal((await readProjectWorkspace(testController(db), "CODE")).git?.dirty, true);
    ...
    await assert.rejects(provisionProjectWorkspace(testController(db), "CODE", { mode: "create" }), { code: "archived" });
    ...
    assert.equal((await readProjectWorkspace(testController(db), "CODE")).workspace?.status, "ready");
    ...
    state = await readProjectWorkspace(testController(db), "RETRY");
    assert.equal(state.error?.code, "interrupted");
    state = await provisionProjectWorkspace(testController(db), "RETRY", null, true);
    ...
    const failure = await provisionProjectWorkspace(testController(db), "BADIMPORT", { mode: "import", sourceUrl: "https://127.0.0.1:1/unreachable.git" });
    assert.equal(failure.workspace?.status, "failed");
    assert.equal(failure.error?.code, "git_failed");
    const retried = await provisionProjectWorkspace(testController(db), "BADIMPORT", null, true);
    ...
    await assert.rejects(provisionProjectWorkspace(testController(db), "BADIMPORT", null, true), { code: "archived" });
    await fs.rename(path.join(location, ".git"), path.join(location, "removed-git"));
    assert.equal((await readProjectWorkspace(testController(db), "CODE")).error?.code, "unavailable");
    await assert.rejects(provisionProjectWorkspace(testController(db), "CODE", { mode: "create" }), { code: "conflict" });
  } finally {
    db.close();
