...
describe("project hubs CRUD", () => {
  ...
  it(
    ...
    withTempDb(async (db) => {
      ...
      const created = await projects.create(db, { key: "demo", title: "Demo Project" });
      ...
      const snapshot = await snapshots.get(db, "DEMO");
      assert.ok(snapshot);
      assert.equal(snapshot.project.key, "DEMO");
      assert.ok(snapshot.nodes.some((node) => node.type === "project"));

      const stats = await projects.stats(db, "DEMO");
      assert.ok(stats);
      assert.equal(stats.byType.project?.total, 1);
      assert.equal(stats.byType.project?.inProgress, 1);
      assert.equal(stats.workflowDefs, 0);

      await assert.rejects(() => projects.remove(db, PROTECTED_PROJECT_KEY), /protected/i);

      const deleted = await projects.remove(db, "DEMO");
      assert.equal(deleted.deleted, "DEMO");
      assert.equal((await projects.list(db)).length, 1);
      assert.equal(await projects.stats(db, "DEMO"), null);
    })
  );
  ...
  it(
    ...
    withTempDb(async (db) => {
      await assert.rejects(() => projects.create(db, { key: "bad-key", title: "X" }), /key/i);
      await assert.rejects(() => projects.create(db, { key: "PLAN", title: "Dup" }), /already exists/i);
      await assert.rejects(() => projects.create(db, { key: "OK", title: "   " }), /title/i);
    })
  );
});
